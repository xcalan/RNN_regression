...
# rnn 测试
def test():
    net_test = torch.load('..\\model\\lstm_model.pkl')  # load model
    hidden_state = torch.zeros([1, 1, 32])  # 隐藏状态初始化(num_layers, batch size, hidden_size)
    c_state = torch.zeros([1, 1, 32])
    test_loss = 0
    net_test.eval()
    with torch.no_grad():
        for idx, (sin_input, cos_output) in enumerate(test_loader):
            ...
            prediction, (hidden_state, c_state) = net_test(sin_input_torch.to(device),
