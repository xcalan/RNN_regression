...
# 2. 构建模型，优化器
lstm = LSTM(seq_len=batch_size_train).to(device)
optimizer = torch.optim.SGD(lstm.parameters(), lr=0.001, momentum=0.8)
...
def train_lstm(epoch):
    hidden_state = torch.zeros([lstm.layer_num, 1, lstm.hidden_num])  # 隐藏状态初始化(num_layers, batch size, hidden_size)
    c_state = torch.zeros([lstm.layer_num, 1, lstm.hidden_num])
    global total_loss
    mode = True
    lstm.train(mode=mode)  # 模型设置为训练模式
    ...
    for idx, (sin_input, cos_output) in enumerate(train_loader):
        ...
        prediction, (hidden_state, c_state) = lstm(sin_input_torch.to(device),
                                                   hidden_state.to(device),
                                                   c_state.to(device))

        # 要把 h_state,c_state 重新包装一下才能放入下一个 iteration, 不然会报错!!!
        hidden_state = Variable(hidden_state.data).to(device)
        c_state = Variable(c_state.data).to(device)
        ...
        if idx == len(train_loader) - 1:
            ...
            if loss_epoch < total_loss:
                ...
                torch.save(lstm, '..\\model\\lstm_model.pkl')  # save model
